...
TFVARS_FILE="deny.tfvars"

# Initialize import script
echo "#!/bin/bash" > "$IMPORTS_FILE"
echo "set -euo pipefail" >> "$IMPORTS_FILE"
echo "# Auto-generated Terraform import commands for Deny policies" >> "$IMPORTS_FILE"
echo "" >> "$IMPORTS_FILE"

# Initialize tfvars file
echo "# Auto-generated variables for Deny policies" > "$TFVARS_FILE"
echo "deny_scp_files = [" >> "$TFVARS_FILE"
...
for pid in $policies; do
  ...
  # Only process policies starting with "Deny"
  ...
  echo ">>> Processing DENY policy: $raw_name (safe filename: $safe_name.json)"

  # SCP import command
  echo "terraform import 'aws_organizations_policy.deny_scp[\"${safe_name}.json\"]' $pid" >> "$IMPORTS_FILE"
  ...
  if [ -n "$targets" ]; then
    for tid in $targets; do
      ...
      echo "terraform import 'aws_organizations_policy_attachment.deny_attach[\"${safe_name}_${tid_clean}\"]' ${pid}:${tid_clean}" >> "$IMPORTS_FILE"
...
echo "deny_attachments = {" >> "$TFVARS_FILE"
...
echo ""
echo "=== Done! Deny policies generated ==="
echo "Files created:"
echo "  - policies/ (policy JSON files)"
echo "  - imports.sh (import commands)"
echo "  - deny.tfvars (variables)"
echo ""
echo "Next step:"
echo "  cd .. && terraform apply -var-file=Deny/deny.tfvars"
